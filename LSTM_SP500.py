...
train_in = np.load(npy_data_dir + 'train_in_SP500.npy')
train_out = np.load(npy_data_dir + 'train_out_SP500.npy')
test_in = np.load(npy_data_dir + 'test_in_SP500.npy')
test_out = np.load(npy_data_dir + 'test_out_SP500.npy')

def LSTM_Model():
    days = 5
    factor_num = 5
    lstm1 = keras.layers.LSTM(40, activation='relu', recurrent_activation='hard_sigmoid',
                             use_bias=True, kernel_initializer='glorot_uniform',
                              return_sequences=True)
    dropout1 = keras.layers.Dropout(rate=0.2)
    lstm2 = keras.layers.LSTM(20, activation='relu', recurrent_activation='hard_sigmoid',
                             use_bias=True, kernel_initializer='glorot_uniform')
...
n_epochs = 300
batch_size = 10
model_name = model_dir + 'LSTM_model.h5'
lrdecay = keras.optimizers.schedules.PolynomialDecay(ini_learning_rate, decay_steps = 3000,
...
plt.savefig(model_dir + 'ELG_SP500.png')
